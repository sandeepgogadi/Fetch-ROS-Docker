...
def transform_to_pose(matrix):
    pose = Pose()
    b_origin = np.zeros((4, 1))
    b_origin[3] = 1.0
    b_origin_in_a = np.matmul(matrix, b_origin)
    # print(b_origin_in_a)
    pose.position.x = b_origin_in_a[0, 0]
    pose.position.y = b_origin_in_a[1, 0]
    pose.position.z = b_origin_in_a[2, 0]
    quarternion = tft.quaternion_from_matrix(matrix)
